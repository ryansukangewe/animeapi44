import type * as ISP from "./interfaces/ISamehadakuParser"
import type * as ISPE from "./interfaces/ISamehadakuParserExtra"
...
export default class SamehadakuParser extends SamehadakuParserExtra {
  parseHome(): Promise<ISP.Home> {
    return this.scrape<ISP.Home>(
      ...
      async ($, data) => {
        ...
        const isEmpty =
          data.recent.animeList.length === 0 && data.batch.batchList.length === 0 && data.movie.animeList.length === 0
    ...
    return this.scrape<ISP.AnimeEpisode>(
      ...
      async ($, data) => {
        ...
        if (data.defaultStreamingUrl.includes("api.wibufile.com")) {
          data.defaultStreamingUrl = originUrl + this.generateHref("/", `wibufile?url=${data.defaultStreamingUrl}`)
        }
        ...
        const fixedServerQualities: string[] = []
  ...
  async parseServerUrl(serverId: string, originUrl: string): Promise<ISP.ServerUrl> {
    ...
    const url = await wajikFetch(
      ...
      (response) => {
        if (!response.ok) throw { status: 400, message: "Bad Request" }
      },
    )
    ...
    if (data.url.includes("api.wibufile.com")) {
      data.url = originUrl + this.baseUrlPath + `/wibufile?url=${data.url}`
    }
